...
import { CurrencyService } from 'app/modules/admin/master-data/general/currency/currency.service'; // Modify import path accordingly
import {
    Currency,
    CurrencyPagination,
} from 'app/modules/admin/master-data/general/currency/currency.types'; // Modify import path accordingly
import {
    Observable,
    Subject,
    debounceTime,
...
@Component({
    selector: 'currency-list', // Update the selector to reflect currency
    templateUrl: './currency.component.html', // Modify the template path if needed
    styles: [
        /* language=SCSS */
        `
            .currency-grid {
...
export class CurrencyListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    currencies$: Observable<Currency[]>; // Updated to Currency
    flashMessage: 'success' | 'error' | null = null;
    isLoading: boolean = false;
    pagination: CurrencyPagination; // Updated to CurrencyPagination
    searchInputControl: UntypedFormControl = new UntypedFormControl();
    selectedCurrency: Currency | null = null; // Updated to Currency
    ...
    constructor(
        ...
        private _currencyService: CurrencyService // Updated to CurrencyService
    ) {}
    // -----------------------------------------------------------------------------------------------------
    // @ Lifecycle hooks
    // -----------------------------------------------------------------------------------------------------
    ...
    ngOnInit(): void {
        ...
        this._currencyService.pagination$
            ...
            .subscribe((pagination: CurrencyPagination) => { // Updated to CurrencyPagination
        ...
        // Get the currency
        this.currencies$ = this._currencyService.currencies$; // Updated to currencies$
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.closeDetails();
                    this.isLoading = true;
                    return this._currencyService.getCurrencies( // Updated to CurrencyService methods
                        0,
                        10,
                        'currencyName',
                        'asc',
    ...
    ngAfterViewInit(): void {
        if (this._sort && this._paginator) {
            ...
            // Get currency if sort or page changes
    ...
    updateSelectedCurrency(): void {
        ...
        this._currencyService
            .updateCurrencyType(currency.id, currency) // Updated to CurrencyService methods
            .subscribe(() => {
